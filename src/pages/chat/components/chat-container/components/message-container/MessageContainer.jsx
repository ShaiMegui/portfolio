...
const MessageContainer = () => {
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        // Appel API pour récupérer les messages de contact
    ...
    const getChannelMessages = async () => {
      try {
        // Assurez-vous d'utiliser l'ID dans l'URL
        const res = await apiClient.get(
          `${GET_CHANNEL_MESSAGES_ROUTE}/${selectedChatData?._id}/messages`,
          { withCredentials: true }
        );
        if (res.data.messages) {
          setSelectedChatMessages(res.data.messages);
  ...
  const checkIfImage = (filePath) => {
    const imageRegex =
      /\.(jpg|jpeg|png|gif|bmp|svg|webp|tiff|tif|ico|heic|heif)$/i;
  ...
  const donwloadFileUrl = async (url) => {
            ...
            {checkIfImage(message.fileUrl) ? (
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <span
                  ...
                  onClick={() => donwloadFileUrl(message.fileUrl)}
                >
                  <IoMdArrowRoundDown />
                </span>
              </div>
      ...
      {showImage && (
        <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col">
          ...
          <div className="flex gap-5 fixed top-0 mt-5">
            <button
              className="bg-black/20 p-3 text-2xl rounded-full hover:bg-black/50 cursor-pointer transition-all duration-300"
              onClick={() => donwloadFileUrl(imageURL)}
            >
              <IoMdArrowRoundDown />
            </button>
            <button
